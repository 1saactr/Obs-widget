							...
							eventData.inputs.forEach((input) => {
								if (input.inputName == obsMicInput) {
									if (input.inputLevelsMul.length == 0) {
										...
										let leftMeter = document.getElementById("volumenIzquierdo");
										let rightMeter = document.getElementById("volumenDerecho");
				...
				case 7:		// RequestResponse OpCode
					switch (data.d.requestType) {
						case "GetStats":
							{
								...
								document.getElementById("statsBasicos").innerHTML = `CPU: ${cpu} • MEM: ${memory} • RENDER TIME: ${averageFrameRenderTime}`;
								document.getElementById("statsAvanzados").innerHTML = `MISSED FRAMES ${outputSkippedFramesPerc} • SKIPPED FRAMES ${renderSkippedFramesPerc}`;
								document.getElementById("valorFPS").innerHTML = `${activeFps}`;
						...
						case "GetVideoSettings":
							{
								...
								let fpsMeter = document.getElementById("medidorFPS");
								var tl = new TimelineMax();
								tl
									.to(fpsMeter, 0.1, { height: + 100 * fpsMeterValue + "%", ease: Linear.easeNone });

								if (fpsMeterValue >= 1)
									document.getElementById("medidorFPS").style.backgroundColor = `#37d247`;
								else if (fpsMeterValue > 0.9)
									document.getElementById("medidorFPS").style.backgroundColor = `#e5af24`;
								else
									document.getElementById("medidorFPS").style.backgroundColor = `#D12025`;
							}
							break;
						case "GetRecordStatus":
							{
								...
								if (responseData.outputActive === false) {
									document.getElementById("estadoGrabacion").innerHTML = ``;
									document.getElementById("tiempoGrabacion").innerHTML = ``;
									document.getElementById("tamanoArchivo").innerHTML = ``;
									document.getElementById("anilloGrabacion").style.visibility = 'hidden';
									document.getElementById("infoGrabacion").style.visibility = `hidden`;
								}
								else {
									document.getElementById("estadoGrabacion").innerHTML = `REC 🔴`;
									document.getElementById("tiempoGrabacion").innerHTML = `${RemoveMilliseconds(responseData.outputTimecode)}`;
									document.getElementById("tamanoArchivo").innerHTML = `${ConvertToMegabytes(responseData.outputBytes)}MB`;
									document.getElementById("anilloGrabacion").style.visibility = 'visible';
									document.getElementById("infoGrabacion").style.visibility = `visible`;
								}
							}
							break;
						case "GetStreamStatus":
							{
								...
								if (responseData.outputActive === false) {
									document.getElementById("anilloStream").style.visibility = 'hidden';
									document.getElementById("infoStream").style.visibility = `hidden`;
								}
								else {
									...
									document.getElementById("bitrateStream").innerHTML = `${Math.floor(kbps)} kb/s`;
									document.getElementById("tiempoStream").innerHTML = `${RemoveMilliseconds(responseData.outputTimecode)}`;
									GetStreamServiceSettings();

									document.getElementById("anilloStream").style.visibility = 'visible';
									document.getElementById("infoStream").style.visibility = `visible`;
								}
							}
							break;
						case "GetStreamServiceSettings":
							{
								...
								switch (responseData.streamServiceSettings.service) {
									case "Twitch":
										document.getElementById("plataforma").innerHTML = "🟣 Twitch";
										break;
									case "YouTube":
										document.getElementById("plataforma").innerHTML = "🔴 YouTube";
										break;
									case undefined:
										document.getElementById("plataforma").innerHTML = "🔴 LIVE";
										break;
									default:
										document.getElementById("plataforma").innerHTML = `🔴 ${responseData.streamServiceSettings.service}`;
										break;
								}
							}
							break;
						case "GetProfileList":
							{
								let responseData = data.d.responseData;
								document.getElementById("nombrePerfil").innerHTML = `Profile: ${responseData.currentProfileName}`;
							}
							break;
						case "GetInputMute":
							{
								...
								if (responseData.inputMuted)
									document.getElementById("iconoMicrofono").style.visibility = `visible`;
								else
									document.getElementById("iconoMicrofono").style.visibility = `hidden`;
							}
							break;
					}
					break;
